...
RUN echo '#!/bin/sh' > /app/start.sh && \
    ...
    echo 'NODE_ENV=production npm_config_cache=/tmp/.npm supergateway --stdio "NODE_ENV=production npm_config_cache=/tmp/.npm npx -y $SERVER_TYPE $DATA_FOLDER" --port $PORT --baseUrl $BASE_URL --ssePath $SSE_PATH --messagePath $MESSAGE_PATH' >> /app/start.sh && \
